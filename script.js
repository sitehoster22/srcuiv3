...
function playMovie(tmdbId) {
    const embedUrl = `/api/proxy?url=${encodeURIComponent(`https://vidsrc.su/embed/movie/${tmdbId}`)}`;
...
function updatePlayer(tmdbId, season, episode) {
    const embedUrl = `/api/proxy?url=${encodeURIComponent(`https://vidsrc.su/embed/tv/${tmdbId}/${season}/${episode}`)}`;
    document.getElementById('player').innerHTML = `
        <iframe src="${embedUrl}" allowfullscreen></iframe>
    `;
