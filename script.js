...
function playMovie(tmdbId) {
    const embedUrl = `https://movie.nightfallen.my/api/proxy?url=embed/movie/${tmdbId}`;
...
function updatePlayer(tmdbId, season, episode) {
    const embedUrl = `https://movie.nightfallen.my/api/proxy?url=embed/tv/${tmdbId}/${season}/${episode}`;
    document.getElementById('player').innerHTML = `
        <iframe src="${embedUrl}" allowfullscreen></iframe>
    `;
